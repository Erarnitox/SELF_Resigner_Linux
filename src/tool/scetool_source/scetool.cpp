...
static s8 *_print_out = NULL;

static std::vector<std::string> s_argv_storage;
...
static void print_version()
{
  ...
  printf("(C) 2025 Erarnitox - Major Patches and improvements");
...
static void parse_args(const std::vector<std::string>& args) {
  ...
  // Convert std::vector<std::string> to char* array for getopt.
  // Keep argv strings alive until the next scetool invocation.
  std::vector<char*> argv_vec;
  
  // Add program name (typically args[0])
  if (not args.empty()) {
    s_argv_storage.push_back("scetool");
  }
    
  // Add all other arguments
  for (size_t i = 0; i < args.size(); ++i) {
    s_argv_storage.push_back(args[i]);
    argv_vec.push_back(const_cast<char*>(s_argv_storage.back().c_str()));
  }
    
  // Null-terminate argv
  argv_vec.push_back(nullptr);
    
  const int argc = static_cast<int>(argv_vec.size() - 1);
